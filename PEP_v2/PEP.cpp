...
void test_3d() {
  {
    ...
    SquareLattice_3 lattice(Lx, Ly, Lz);
    lattice.create_particles_randomly(par_arr, myran, phi);
    std::vector<size_t> seq;
    seq.reserve(par_arr.size());
    for (int i = 0; i < par_arr.size(); i++) {
...

  {
    ...
    char logfile[100];
    char xyfile[100];
    snprintf(logfile, 100, "%d_1.log", Lx);

    snprintf(xyfile, 100, "%d_1.extxyz", Lx);
    LogExporter log(logfile, 0, n_step, 1000, par_arr.size());
    XYExporter xy(xyfile, 0, n_step, 1000, Lx, Ly);
    ...
    for (int i = 1; i <= n_step; i++) {
      shuffle(seq, myran);
      update_in_rand_seq1(par_arr, lattice, myran, seq, beta);
      log.record(i);
      if (i % 100 == 0) {
        std::cout << i << std::endl;
      }
...

  {
    ...
    char logfile[100];
    char xyfile[100];
    snprintf(logfile, 100, "%d_2.log", Lx);

    snprintf(xyfile, 100, "%d_2.extxyz", Lx);
    LogExporter log(logfile, 0, n_step, 1000, par_arr.size());
    XYExporter xy(xyfile, 0, n_step, 1000, Lx, Ly);
    ...
    for (int i = 1; i <= n_step; i++) {
      shuffle(seq, myran);
      update_in_rand_seq2(par_arr, lattice, myran, seq, beta);
      log.record(i);
      if (i % 100 == 0) {
        std::cout << i << std::endl;
      }
...

  {
    ...
    char logfile[100];
    char xyfile[100];
    snprintf(logfile, 100, "%d_3.log", Lx);

    snprintf(xyfile, 100, "%d_3.extxyz", Lx);
    LogExporter log(logfile, 0, n_step, 1000, par_arr.size());
    XYExporter xy(xyfile, 0, n_step, 1000, Lx, Ly);
    ...
    for (int i = 1; i <= n_step; i++) {
      shuffle(seq, myran);
      update_in_rand_seq3(par_arr, lattice, myran, seq, beta);
      log.record(i);
      if (i % 100 == 0) {
        std::cout << i << std::endl;
      }
